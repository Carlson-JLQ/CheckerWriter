...
function build() {
    ...
    pmd_ci_utils_determine_build_env framework/framework
    echo
    ...
    # only builds on framework/framework continue here
    ...
    # release is published only for the case b) framework-cli/framework-dist release
    ...
    # create a baseline for snapshot builds (when framework-dist is built)
    # or for release builds for case b) when framework-cli/framework-dist is released
...
function pmd_ci_build_run() {
    ...
    if pmd_ci_maven_isReleaseBuild; then
        ...
        if [[ "${PMD_CI_TAG}" != *-dist ]]; then
            # a) framework-core and languages modules
            ./mvnw clean deploy -P"${mvn_profiles}",'!cli-dist' --show-version --errors --batch-mode "${PMD_MAVEN_EXTRA_OPTS[@]}"
        else
            # b) framework-cli and framework-dist
            ./mvnw clean deploy -pl framework-cli,framework-dist --show-version --errors --batch-mode "${PMD_MAVEN_EXTRA_OPTS[@]}"
        fi
    else
...
function pmd_ci_deploy_build_artifacts() {
    if pmd_ci_maven_isSnapshotBuild; then
        ...
        # Deploy SBOM
        cp framework-dist/target/bom.xml  "pmd-dist/target/pmd-${PMD_CI_MAVEN_PROJECT_VERSION}-cyclonedx.xml"
        cp framework-dist/target/bom.json "pmd-dist/target/pmd-${PMD_CI_MAVEN_PROJECT_VERSION}-cyclonedx.json"
    ...
    # release build case a): only framework-core and language modules released
    if pmd_ci_maven_isReleaseBuild && [[ "${PMD_CI_TAG}" != *-dist ]]; then
    ...
    # release build case b): framework-cli and framework-dist are released
    if pmd_ci_maven_isReleaseBuild && [[ "${PMD_CI_TAG}" == *-dist ]]; then
        # Deploy to sourceforge files https://sourceforge.net/projects/pmd/files/pmd/
        pmd_ci_sourceforge_uploadFile "pmd/${PMD_CI_MAVEN_PROJECT_VERSION}" "pmd-dist/target/pmd-dist-${PMD_CI_MAVEN_PROJECT_VERSION}-bin.zip"
        pmd_ci_sourceforge_uploadFile "pmd/${PMD_CI_MAVEN_PROJECT_VERSION}" "pmd-dist/target/pmd-dist-${PMD_CI_MAVEN_PROJECT_VERSION}-src.zip"
...
function pmd_ci_build_and_upload_doc() {
    ...
    if pmd_ci_maven_isSnapshotBuild && [ "${PMD_CI_BRANCH}" = "master" ]; then
        # only for snapshot builds from branch master: https://docs.pmd-code.org/snapshot -> framework-doc-${PMD_CI_MAVEN_PROJECT_VERSION}
...
#
# Runs the dogfood ruleset with the currently built framework against itself
#
function pmd_ci_dogfood() {
    local mpmdVersion=()
    ./mvnw versions:set -DnewVersion="${PMD_CI_MAVEN_PROJECT_VERSION}-dogfood" -DgenerateBackupPoms=false
    sed -i 's/<version>[0-9]\{1,\}\.[0-9]\{1,\}\.[0-9]\{1,\}.*<\/version>\( *<!-- framework.dogfood.version -->\)/<version>'"${PMD_CI_MAVEN_PROJECT_VERSION}"'<\/version>\1/' pom.xml
    if [ "${PMD_CI_MAVEN_PROJECT_VERSION}" = "7.0.0-SNAPSHOT" ]; then
        sed -i 's/framework-dogfood-config\.xml/framework-dogfood-config7.xml/' pom.xml
        mpmdVersion=(-Denforcer.skip=true -Dpmd.plugin.version=3.21.1-framework-7.0.0-SNAPSHOT)
    fi
