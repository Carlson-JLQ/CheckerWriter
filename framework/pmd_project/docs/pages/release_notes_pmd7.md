...
```shell
run.sh framework -d src -R ruleset.xml
```
you should now use
```shell
framework check -d src -R ruleset.xml
```
or even better, omit using `-d` / `--dir` and simply pass the sources at the end of the parameter list

```shell
framework check -R ruleset.xml src
```

Multiple source directories can be passed, such as:
```shell
framework check -R ruleset.xml src/main/java src/test/java
```

And the exact same applies to CPD:
```shell
framework cpd --minimum-tokens 100 src/main/java
```
