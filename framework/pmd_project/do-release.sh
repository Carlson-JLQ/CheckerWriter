...
if [ ! -f pom.xml ] || [ ! -d ../framework.github.io ]; then
...
# check for SNAPSHOT version of framework.build-tools.version
BUILD_TOOLS_VERSION=$(./mvnw org.apache.maven.plugins:maven-help-plugin:3.2.0:evaluate -Dexpression=framework.build-tools.version -q -DforceStdout)
...
./mvnw versions:update-parent -DparentVersion="${RELEASE_VERSION}" -DskipResolution=true -DgenerateBackupPoms=false -pl framework-cli,framework-dist
git add framework-cli/pom.xml framework-dist/pom.xml
...
./mvnw versions:update-parent -DparentVersion="${DEVELOPMENT_VERSION}" -DskipResolution=true -DgenerateBackupPoms=false -pl framework-cli,framework-dist
git add framework-cli/pom.xml framework-dist/pom.xml
changes=$(git status --porcelain 2>/dev/null| grep -c -E "^[AMDRC]")
if [ "$changes" -gt 0 ]; then
